...
class TestGlusterDReserveLimitAndRM(GlusterBaseClass):
    """ Testing Reserve limit in GlusterD """
    ...
    def test_glusterd_reserve_limit_and_rm(self):
        """
        Test GlusterD Reserve limit and rm the files.
        ...
        4. Create files on mountpoint till the storage limit reaches 100%
        5. Delete all the data on mountpoint.
        """
        ...
        # Log volume info and status before setting reserve option
        ...
        # Log volume info and status after setting reserve options
        log_volume_info_and_status(self.mnode, self.volname)
        ...
        space_on_mount = int(out)

        # Computing the number of 1 KB files that'd violate the limit set.
        file_count = round(space_on_mount)+10
        ...
        # Wait for IO to fail as reserve limit is violated.
        self.assertFalse(wait_for_io_to_complete(self.all_mounts_procs,
                                                 self.mounts[0]),
                         "IO should have failed!")
        g.log.info("IO failed due to reserve limit being breached.")

        # The available space at mountpoint should be 0.
        cmd = ("df --output=avail %s | grep '[0-9]'"
               % self.mounts[0].mountpoint)
        ret, out, err = g.run(self.mounts[0].client_system, cmd)
        self.assertEqual(int(out), 0, "Memory usage is not 100%.")
        g.log.info("Memory usage is 100%")

        # Delete all files at mountpoint
        cmd = ("cd %s/; rm -rf *" % (self.mounts[0].mountpoint))
        ret, _, _ = g.run(self.clients[0], cmd)
        self.assertEqual(ret, 0, "Failed to remove the files from mountpoint")
        g.log.info("Removed all files from mountpoint")
